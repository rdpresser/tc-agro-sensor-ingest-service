...
namespace TC.Agro.SensorIngest.Application.UseCases.GetLatestReadings
{
    public sealed record GetLatestReadingsQuery : ICachedQuery<PaginatedResponse<GetLatestReadingsResponse>>
    {
        ...
        public string GetCacheKey => _cacheKey ?? $"GetLatestReadingsQuery-{OwnerId}-{SensorId}-{PlotId}-{PageNumber}-{PageSize}";
        ...
        public void SetCacheKey(string cacheKey)
            => _cacheKey = $"GetLatestReadingsQuery-{OwnerId}-{SensorId}-{PlotId}-{PageNumber}-{PageSize}-{cacheKey}";
    }
}
